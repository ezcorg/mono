...
**witmproxy on icanhaz: the options, for a decision (2026-09-14).** M1b decided to share the library, not the daemon; after M3 the question is what "witmproxy's plugin capabilities become icanhaz providers" should mean. Three shapes, in increasing coupling: (a) **shared grant store only**: witmproxy embeds `icanhaz_host::broker::GrantStore` so every plugin capability instance is a grant with a token, principal (`installed-app` = the plugin id), scope, sturdy references and durability; witmproxy's providers stay in-process and call `admit` exactly as icanhaz's native providers do; gain: one audit view, `certify`/`redeem` for plugin authority, one persistence path; cost: witmproxy takes a dependency on `icanhaz-host` (wasmtime is already there; wtransport, iroh and reqwest are not, so `icanhaz-host` would need a `broker`-only feature). (b) **icanhaz as the consent surface**: witmproxy's install-time consent and its registry UI ask the icanhaz tray app (over the loopback broker) instead of their own page, so a plugin's `wants` are granted where every other grant is, and the tray app's forms render plugin configuration through the shared `forms` schema; requires (a). (c) **witmproxy plugins as icanhaz components**: `witmproxy:plugin/capabilities` becomes a thin world over `icanhaz:nocap` interfaces and the proxy's request events flow through icanhaz's component server; the logger's call rate (thousands per second) argues against crossing a process boundary, so this only works with icanhaz embedded, at which point it is (a)+(b) with the proxy's own capability kinds (`annotator`, `local-storage`, `clock`, `logger`) registered in icanhaz's capability registry. Recommendation: (a) now with a `broker` feature on `icanhaz-host`, (b) once the tray app is the daily consent surface, (c) never as a separate process. Not started; awaiting a decision.
