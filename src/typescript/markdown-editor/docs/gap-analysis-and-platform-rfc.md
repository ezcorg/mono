...
|---|---|---|---|
| 1 Narrow | edits `when` / `allow` in the consent window or scope editor | generic CEL membrane | none |
| 2 Compose | wraps an existing capability, or brings a component built with `cargo component`, componentize-js, componentize-py or MoonBit | validated, content-addressed, composed with wac at grant time | the language's own toolchain, via the scaffold's `flake.nix` |
| 3 Replace a native provider | builds icanhaz from source | — | none |

**Our job is that technical users and language models can author and test these.** The CLI is the surface, under the `capability` noun:

- `icanhaz capability new <name>`: a novel capability; pick the interface it exports and the interfaces it imports, get a scaffold with the WIT vendored, a build, and an instructions file for an agent (build, the composition rule, how to add to the store).
- `icanhaz capability wrap <interface>`: the scaffold for sitting in front of an existing capability, importing and exporting the same interface; function-only interfaces start as a generated passthrough, resource interfaces as typed stubs listing the methods to forward.
- `icanhaz capability compose <a.wasm> <b.wasm> …`: the mechanism itself, wac with the daemon's roots, for components that already exist.
- `icanhaz capability inspect <wasm>`, `add`, `list`, `get`: the validator and the store from the command line. `inspect` is the author's first feedback: "import `acme:regex/engine` is not a capability interface: compose your libraries in".

Testing is the component's own language's job, not the CLI's; the scaffold ships with the language's test setup. Tooling is layered so nobody pays for another language: a base `flake.nix` provides the language-agnostic core (wasm-tools, wac, the wit-bindgen CLI), and each scaffold's own devShell adds its language (cargo-component and the wasip2 target for Rust, jco for JavaScript), with a container as the fallback for machines without nix.

**Reproducibility is a property, not a gate.** The store records provenance when an author supplies it (source, commit, build command, builder image) and marks a component reproducible only once a rebuild matched its hash; local work may add unverified components, and publishing or vetting is where the flag becomes a requirement. The daemon and wasmtime are the trusted computing base and are **not** editable in-app; the shipped passthrough, the pathjail and the generic membrane stay ours and reproducible, so the trusted base does not grow as users author. What the app exposes is every component's source when provenance is registered, the WIT of every interface and its CEL environment, and "fork this capability" copies registered source into a scaffold. User-authored capabilities compile to **wasm, not native**, even when the user is the author: composition, portability across executors, sharing and content-addressing, and bounding the user's own bugs all depend on it; native code belongs only to the providers at the bottom.
...
**M1. witmproxy adopts it.** **Status, 2026-09-14, branch `ezcap`:** landed. `witmproxy:plugin@0.0.8` uses `ezco:ezcap/types.scope` (`expression` became `when`; stored rows with the old shape still load through a serde alias), the four provider resources return `result<T, capability-error>`, `build.rs` embeds their CEL environments, a `Membranes` registry mints one instance per granted provider capability at registration (type-checking `allow`), and every `logger`, `local-storage`, `clock` and `annotator` call is admitted with `call.method`, `call.args.*` and `caller.plugin` bound. Event capabilities keep their `when` against the event environment. The in-repo plugins, the scaffold template and the API summary (`when`/`allow`) are updated. `capability-scope` becomes `ezco:ezcap/types.scope` (`expression` maps to `when`); `allow` evaluated per call on logger, annotator, local-storage and clock; resource methods return `result<T, capability-error>`; counters move from the per-event `CapabilityProvider` to registry-owned grants; the web UI scope editor becomes "add a condition" rendered as sentences. Tests: the noshorts plugin narrowed to `key.startsWith("seen/")` with the denial path exercised end to end; an existing manifest with only `expression` loads unchanged. First PR: `capability-scope` gains `allow` (default `"true"`), the four provider resources become membranes over `ezcap::Membrane`, and `CapabilityProvider::build` mints instances from registry-owned grants instead of cloning per event.
...
**M2. icanhaz instances, generic membrane, sturdy references.** (icanhaz) **Status, 2026-09-14, branch `ezcap`:** the first slice is in. `broker.wit` gains `request-scoped(want, scope, reason, pairing)` and `narrow(token, extra)`, with `request` now the unrestricted case; `denied` gains `invalid-scope` and `out-of-scope(sentences)`. The host embeds CEL environments generated from `../wit` at build time (`host/build.rs`), one per grant kind: `filesystem` is the union of the descriptor, `watch` and `workspace` interfaces. Every grant is a membrane instance; `process.spawn`, `watch.open`, `workspace.root-path` and `terminal.open` admit each call with `call.method`, `call.args.*` and `caller.origin` bound; revoking a parent cascades to narrowed children. The filesystem passthrough now admits per operation too: the policy component's `gate.admit(grant, method, args)` reports every descriptor method with its string arguments (`path`, `old-path`, `new-path`), and the host evaluates the grant's `allow` with `call.method` and `call.args.*` bound, so `call.args.path.startsWith("src/")` on a filesystem grant confines `open-at` and friends. Remaining: sturdy references (`restore`), consent-window sentence rendering and clause appending, and the web client's use of the new calls beyond regenerated bindings. **Later the same day:** the consent window and the console prompt render a scoped request's clauses as sentences (`ScopeText`), and the window takes an extra `allow` clause, checked live against the kind's interface (`GrantStore::check_scope`, `Membranes::check`) and applied as an `ezcap::Narrowing` the broker conjoins, so a surface can tighten but never widen. Sturdy references landed as certificates (M3) rather than a bare `restore`; the web client's certificate test exercises `request-scoped`, `certify` and `redeem`. Instance table replaces `GrantStore`; `broker.request` takes `ezco:ezcap/types.capability`; a generic CEL membrane component per shipped interface, generated at build, replaces ad-hoc attenuation, with deep wrapping of returned resources; `narrow(instance, clause)` for pledge-style self-narrowing; `restore(sturdyref)`; the consent window renders sentences. Tests: `fs-passthrough` under the generic membrane with `allow: call.args.path.startsWith("src/")`; a browser test that a denied call surfaces `denied(sentence)`; restore by the wrong audience fails.

**M1b. Shared code between the hosts (2026-09-14, branch `ezcap`):** `ezcap::Membranes` and `ezcap::build::write_envs` replace the two hosts' duplicate registries and build scripts; `ezco:ezcap/forms` carries the declared-configuration types (with `input-type.secret`) that witmproxy's manifests define and icanhaz's provider configuration will use; `src/rust/ezdb` is the encrypted SQLite store (SQLCipher) lifted from witmproxy with a `KeySource` that uses the OS keychain when there is one and a 0600 key file otherwise, and witmproxy now falls back to it when no `--db-password` is given. Decided: share the library, not the daemon. witmproxy keeps its in-process capability wiring (its logger is called thousands of times a second) and its own `Consent`; once M3 makes grants durable, its plugin capabilities become icanhaz providers and `witmproxy:plugin/capabilities` a thin world over icanhaz interfaces.

**M2b. `inference` capability in icanhaz.** **Status, 2026-09-14, branch `ezcap`:** landed. `icanhaz:nocap/inference` (`complete` streams `[kind][len][payload]` frames: text deltas, a final usage record, or an error; `models` lists what the grant covers) behind `capability-kind.inference { models }`. Backends: OpenAI-compatible HTTP (Ollama, LM Studio, OpenRouter, vLLM), Anthropic Messages, and an `echo` loopback for demos and tests. Configuration and state go through two generic store tables, not capability-owned ones: providers are declared configuration under `inference/<name>` (kind, base_url, api_key as a `secret`, models), the analogue of witmproxy's plugin configuration; per-provider per-day token spend is per-owner state, the analogue of a plugin's local-storage. Budgets are scope clauses over `state.tokens` (this grant) and `state.day_tokens` (today's spend through the provider, synced from the store before each admission), charged from the provider's own usage report after each completion. The daemon opens `~/.icanhaz/icanhaz.db` (`ICANHAZ_DB`, key from `ICANHAZ_DB_KEY`, else keychain, else key file) and runs without it if it cannot; environment variables (`ICANHAZ_ANTHROPIC_API_KEY`, `ICANHAZ_OPENAI_BASE_URL`, `ICANHAZ_ECHO`) configure providers for headless use. Remaining: the consent app's provider form (the schema is declared host-side; the `forms` WIT plumbing to the tray UI is next) and tool use. `icanhaz:nocap/inference` with a providers config (`~/.icanhaz/providers.toml`: Anthropic, OpenAI-compatible, Ollama, the `claude`/`codex` CLIs, any process), streamed completions over wRPC, `state.tokens` counters, model and budget as `allow` clauses. Tests: a grant scoped to one model refuses another; a budget clause denies past its limit; witmproxy can request the same capability from the daemon for a plugin that wants it. **Later the same day:** declared configuration is generic: a capability registers a `forms` schema (`configuration::Declared`) in the capability registry, instances live under `<id>/<instance>` owners in the generic `configuration` table, the shared module validates writes, masks secrets on read and keeps a stored secret the form left blank, and the consent app's capabilities tab renders each schema as an add/edit/remove form; the inference provider reloads after a write. The store and configured providers travel as `daemon::Services`, built by the binary or the tray app.
...
**M3. Certificates and share bundles.** Ed25519-signed link chain; `redeem(cert)` with audience checked against the `wrpc-transport-iroh` connection context; pairing as the persisted form of a redeemed certificate; adopt `wrpc-transport-iroh` from djt (publish or vendor beside `src/rust/wrpc`). Tests: a bundle redeemed by its audience; the same bundle refused for another key; an appended link that widens is rejected at verification. **Status, 2026-09-14, branch `ezcap`:** landed in three slices. (1) `ezcap::cert`: a certificate is a sturdy reference to a minted instance plus an audience (`any` / `origin` / `peer` key), an expiry and a chain of appended narrowings, each link Ed25519-signed over the previous signature, wire form `ezcap1.<base64url JSON>`; the broker gains `certify(token, audience, ttl, extra)`, `redeem(cert)` and `identity()`; a certificate never carries the bearer token (the store maps a sturdy id to the grant); redemption checks issuer, chain, expiry and the audience as the transport proved it, then issues a grant narrowed by the chain and living no longer than certificate or source; revoking the source voids every certificate on it; the broker's key persists under `broker/identity`. (2) The peer path: djt's `wrpc-transport-iroh` vendored at `src/rust/wrpc-transport-iroh`; the daemon binds an iroh endpoint (ALPN `icanhaz/0`) whose secret key *is* the broker identity, so a certificate's issuer is its locator; the handshake-proven endpoint id is `ReqCtx.peer`, a `peer` audience is checked against exactly that, grants issued over iroh bind to the peer key as principal, and peers pass the hosts allowlist like origins. (3) Pairings and hosts moved into the store (`broker/pairings`, `broker/hosts`), the JSON dotfiles imported once and removed. Not yet: share bundles as an editor object (E4), offline attenuation from the browser (a WebCrypto key), and certified grants persisting across a daemon restart. **Later (2026-09-14):** tool use in `inference.complete` (`tools`, `tool-calls`/`tool-call-id` turns, a kind-3 frame; the host only relays, the caller runs the tool with its own grants; OpenAI and Anthropic mapped, echo loops deterministically). Certified grants persist (`broker/grant:<token>` rows restored at startup with their sturdy ids), so a certificate survives the owner's daemon restarting. **Share bundles** exist as a client primitive in `@joinezco/icanhaz-web` (`share.ts`: `createBundle`, `openBundle`, `decodeBundle`, `certificateRoot`; wire `ezbundle1.<base64url JSON {v, document, issuer, grants:[{cert, summary}]}>`): a bundle holds certificates, never tokens; opening it redeems each at the issuing broker and reports per-certificate refusals; a bundle from another broker is refused until M5 lets the local daemon redeem over iroh and proxy the grant. The editor-side object (a share link in a note, E4) waits on E1's document identity.
...
**M4. Authoring.** The `icanhaz capability` CLI: `inspect`, `add`, `list`, `get` over the store; `compose` with wac; the `new` and `wrap` scaffolds (Rust first, then JS, Python, MoonBit) with vendored WIT, a language devShell and an agent instructions file; the base `flake.nix`; provenance and the reproducible flag in the store. The scripting tier was dropped on 2026-09-17 (see §14). Tests: a wrapped passthrough that refuses writes outside a prefix, built from the scaffold and composed; a component with a stray library import rejected with the composition hint. **Status, 2026-09-17, branch `ezcap`: landed except the language variants.** The CLI (`inspect`, `add` with provenance, `list`, `get`, `compose`, `new`, `wrap`), the Rust scaffold (vendored WIT, devShell, `AGENTS.md`, `src/lib.rs` generated from the WIT with wit-bindgen's borrowing rule applied; typed stubs where resources or streams appear), the base `flake.nix`, and provenance with the `reproducible` flag in the store. Proven end to end on `wrap icanhaz:nocap/workspace@0.1.0`: build, inspect, compose two copies, inspect the composition. Earlier the same day the Tier 3 store and validator landed (below). Not done: JS, Python and MoonBit scaffolds; the rebuild that sets `reproducible`; view-source and "fork this capability" in the app; the resolver (choosing a stored component in the consent window and composing it with wac at grant time), which is the next piece. **Status, 2026-09-16, branch `ezcap`: the Tier 3 half landed.** `icanhaz:nocap/components`: capability components a user brings, validated and kept by hash (`sha256:<hex>`), bytes beside the daemon's database and metadata in the generic state table. The validator decodes the component's world with `wit-parser` and applies the Tier 3 rule: every import must be a capability interface (`wasi:*`, `icanhaz:nocap`, `icanhaz:fspass`, `ezco:ezcap`), a library import is refused naming it with the hint to compose it in at build time, and the exports must satisfy a capability interface. Anyone may add or fetch a component (code is not authority); only local hosts may remove one. The daemon seeds the store with the shipped fs-passthrough so what it links today is addressable by hash like anything a user brings, and the tray lists the store. Not done: linking a stored component in place of a native provider at grant time (that is the resolver, M5's other half), the scaffold and view-source, and all of Tier 2, which needs a decision on the JavaScript engine component (componentize-js/StarlingMonkey is the maintained path but ships a ~10 MB engine per script unless the engine is shared as a separate component; a QuickJS component with a generic `dispatch` export is smaller but nobody maintains one for the component model today).

**M5. Import resolution and remote providers.** The resolver from §15; fetch by hash from bundle, vault, registry and peer; remote provider over wRPC with consent; "no provider" UX listing sources. Tests: a component whose import resolves through a store component two levels deep; a browser-executor plugin whose `fs` import is polyfilled from the daemon. **Status, 2026-09-16, branch `ezcap`: the remote-provider half landed.** `broker.wit` gains `locator()` (`iroh:<identity>?addr=…`), `inspect(token)` and `redeem-at(locator, cert)`: the local daemon connects to the issuer over iroh as itself, redeems the certificate (its audience is the recipient daemon's identity), asks what the grant is, and holds it; the caller gets a local token whose grant names the remote and carries the remote's scope, so admission here mirrors the issuer's. Inference and process forward calls on such a grant, streams included, and usage is charged at both ends. `host/src/remote.rs` keeps one iroh connection per locator; the broker crate stays transport-free behind a `RemoteBroker` trait. Share bundles carry the issuer's locator, and `openBundle` redeems at it through the connected daemon when the issuer is another broker. Not yet proxied: terminal, watch, workspace and the filesystem (its descriptors are per-connection resources, so a proxy has to re-mount at the remote and relay descriptor handles); the import resolver and the component store are the M4/M5 remainder.
**witmproxy on icanhaz (decided and landed 2026-09-15).** The question after M3 was what "witmproxy's plugin capabilities become icanhaz providers" should mean. Three shapes were weighed: (a) a shared grant store, providers staying in-process; (b) icanhaz's tray app as witmproxy's consent and configuration surface; (c) witmproxy plugins as icanhaz components, only viable embedded because of the logger's call rate. Decision: (a) now, (b) once the tray app is the daily surface, (c) never as a separate process. Rather than a cargo feature (there is one user and no compatibility to keep), the broker became its own crate: `icanhaz-broker` (grant store generic over a `GrantKind`, certificates, consent, pairings, hosts, the durable store, declared configuration, the capability registry, the wRPC broker handler) with `icanhaz-host` as the daemon on top of it. witmproxy embeds `icanhaz-broker`: its registry holds a `GrantStore<PluginKind>`, a plugin's granted provider capability is issued as a grant held by the plugin as an `installed-app` principal, re-scoping revokes and reissues, and every minted resource admits calls through its grant token. Plugin authority now lists, narrows, certifies and revokes like every other grant; nothing crosses a process boundary. **The tray app as witmproxy's consent and configuration surface (landed 2026-09-15).** *Consent:* the broker gains `consent(capability, summary, reason, requester)`: another host puts a kind it provides itself (known here only by WIT path, `witmproxy:plugin/capabilities.logger`) to the human, who sees the kind, the scope as sentences and the reason and may append clauses; the conjoined scope comes back and nothing is granted at icanhaz. witmproxy's install paths (`plugin add`, the web upload) replace "grant everything" with a per-capability decision at the tray app when `--plugins-icanhaz ws://127.0.0.1:7777` is set: an approval grants with the human's scope, type-checked when witmproxy issues the grant in its own store; a denial leaves it ungranted; an unreachable daemon grants nothing; without the flag everything is granted as proposed, as before. *Configuration:* `icanhaz:nocap/configuration` lets a local host declare a `forms` schema (`declare`), and read the values back unmasked with a revision (`configured`); browser origins are refused. The broker crate's `configuration::Registry` holds the daemon's own schemas plus the stored declarations and bumps a per-prefix revision on every write. The tray app lists other hosts' declarations under "from other hosts on this machine" and renders `single` declarations as one form without a name. witmproxy declares each plugin's manifest inputs as `witmproxy:<plugin id>` on registration and load, takes the tray-saved values over its own database, and polls the revision every three seconds so an edit at the tray reaches the plugin on its next event (a copy-on-write swap of the plugin's configuration). Removing a plugin withdraws its declaration (the saved values stay in the store until removed there, since they may hold a key the user wants back on reinstall). Not done: the web UI's own configuration page still writes witmproxy's database, which the daemon's values override when a daemon is configured.
