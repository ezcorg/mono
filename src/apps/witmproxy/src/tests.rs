mod tests {
    use crate::test_utils::{create_client, create_echo_server, create_witmproxy, register_test_component, EchoResponse, Protocol};
    ...
    async fn e2e_test() {
        ...
        let client = create_client(ca, &format!("http://{}", proxy.proxy_listen_addr().unwrap()), Protocol::Http2).await;
        let resp = client
            .get(&format!("https://127.0.0.1:{}/example", target.listen_addr().port()))
            .send()
            .await
            .unwrap();
        ...
        let json: EchoResponse = serde_json::from_str(&response_text)
            .map_err(|e| {
                panic!("Failed to parse JSON response: {}. Response body was: '{}'", e, response_text);
            })
            .unwrap();
        ...
        assert!(headers.get("witmproxy").unwrap().to_str().unwrap().contains("res"));
    }
}