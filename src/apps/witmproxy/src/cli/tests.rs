mod tests {
    use crate::{Db, Runtime, cli::Cli, config::{confique_partial_app_config::PartialAppConfig}, plugins::WitmPlugin};
    ...
    async fn test_witm_plugin_add_local_wasm() {
        ...
        let wasm_path = "/home/theo/dev/mono/target/wasm32-wasip2/release/wasm_test_component.signed.wasm";
        
        // Check if the test WASM file exists before running the test
        if !std::path::Path::new(wasm_path).exists() {
            panic!("WASM test component not found at {}, build it first", wasm_path);
        }
        ...
        match result {
            Ok(()) => {
                ...
                assert!(!plugins.is_empty(), "No plugins found in database after adding");
                
                // Check that at least one plugin was added
                let test_plugin = plugins.iter().find(|p| p.name.contains("test") || p.namespace.contains("test"));
                assert!(test_plugin.is_some(), "Test plugin not found in database");
                
                if let Some(plugin) = test_plugin {
                    assert!(!plugin.component_bytes.is_empty(), "Plugin component bytes should not be empty");
                    assert!(!plugin.publickey.is_empty(), "Plugin should have a public key");
                }
            }
            Err(e) => {
    ...
    async fn test_witm_plugin_add_non_wasm_file() {
        ...
        assert!(result.unwrap_err().to_string().contains("Only .wasm files are supported"));
    }
    ...
    async fn test_witm_plugin_remove_by_name() {
        ...
        assert!(remove_result.is_ok(), "Failed to remove plugin: {:?}", remove_result.err());

        // Verify plugin was removed
        let plugins_after = WitmPlugin::all(&mut db, &runtime.engine).await.unwrap();
        assert!(plugins_after.is_empty(), "Plugin was not removed from database");
    }
    ...
    async fn test_witm_plugin_remove_nonexistent() {
        ...
        assert!(remove_result.is_err(), "Should fail when removing nonexistent plugin");
        assert!(remove_result.unwrap_err().to_string().contains("No plugin found"));
    }
}