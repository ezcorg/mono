...
use crate::wasm::bindgen::witmproxy::plugin::capabilities::{ContextualResponse as WasiContextualResponse, RequestContext};
...
impl Event for ContextualResponse
{
    ...
    fn bind_to_cel_activation<'a>(&'a self, activation: cel_cxx::Activation<'a>) -> Option<cel_cxx::Activation<'a>> {
        activation.bind_variable("request", CelRequest::from(&self.request)).ok().and_then(|activation| {
