use super::{
    EventType, PluginAction, PluginMetadata, PluginState, RequestContext,
...
impl WasmPlugin {
    ...
    async fn call_event_handler(
        ...
        let context_ptr = alloc_func.call_async(&mut *store, context_json.len() as i32).await?;
    ...
    pub async fn get_metadata(&self) -> WasmResult<PluginMetadata> {
        ...
        match func {
            ...
            Err(_) => Err(WasmError::InvalidFormat("No get_metadata function found".to_string())),
        }
    }
}
