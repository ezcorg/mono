...
use ezcap::{Membrane, Narrowing, Scope as EzScope};
...
/// Per-kind admission membranes. One [`Membrane`] per grant kind tag
/// (`filesystem`, `process`, `terminal`), each over the CEL environment
/// generated from `../wit` at build time (see `build.rs`): a scope is
/// type-checked against the real interface when a grant is issued, and its
/// `allow` clause is evaluated before every native operation on that grant.
pub struct Membranes {
    by_tag: HashMap<String, Membrane>,
}

impl Membranes {
    /// The environments `build.rs` embedded. A kind with no environment
    /// (`sockets`, until it has an interface) admits only unrestricted scopes.
    pub fn builtin() -> Self {
        static ENVS_JSON: &str = include_str!(concat!(env!("OUT_DIR"), "/ezcap-envs.json"));
        let envs: Vec<(String, ezcap::CallEnv)> = serde_json::from_str(ENVS_JSON)
            .expect("ezcap-envs.json (generated by build.rs) parses");
        let by_tag = envs
            .into_iter()
            .map(|(tag, env)| {
                let membrane = Membrane::new(env, &[]).unwrap_or_else(|e| {
                    panic!("ezcap environment for `{tag}` does not declare: {e}")
                });
                (tag, membrane)
            })
            .collect();
    ...
    /// The generated environment for a kind tag, for documentation and tests.
    pub fn env(&self, tag: &str) -> Option<&ezcap::CallEnv> {
        self.by_tag.get(tag).map(Membrane::env)
    }
}
...
impl GrantStore {
    ...
    pub fn issue_scoped(
        ...
        let instance = match self.membranes.by_tag.get_mut(tag) {
            Some(membrane) => {
    ...
    pub fn admit(&mut self, token: &str, call: AdmitCall) -> Result<(), Denied> {
        ...
        let membrane = self
            ...
            .by_tag
            .get_mut(tag)
            .ok_or(Denied::NotAuthorized)?;
        match membrane.admit(&id, &call) {
            Ok(()) => Ok(()),
    ...
    pub fn admit_grant(&self, token: &str) -> Result<(), Denied> {
        ...
        let membrane = self
            .membranes
            .by_tag
            .get(kind_tag(&grant.kind))
            .ok_or(Denied::NotAuthorized)?;
        if membrane.admit_event(id, Ok) {
            Ok(())
        } else {
    ...
    pub fn narrow_grant(&mut self, token: &str, extra: Narrowing) -> Result<String, Denied> {
        ...
        let tag = kind_tag(&kind);
        let instance = match (instance, self.membranes.by_tag.get_mut(tag)) {
            (Some(parent_id), Some(membrane)) => Some(
                membrane
                    .narrow(&parent_id, &extra)
                    .map_err(|e| Denied::InvalidScope(e.to_string()))?,
            ),
            (None, _) if is_unrestricted(&scope) => None,
            _ => {
                return Err(Denied::InvalidScope(format!(
                    "no admission environment for `{tag}` capabilities"
                )));
        ...
        match self.grants.remove(id) {
            Some(grant) => {
                ...
                if let Some(instance) = &grant.instance {
                    if let Some(membrane) = self.membranes.by_tag.get_mut(kind_tag(&grant.kind)) {
                        membrane.revoke(instance);
                    }
                }
